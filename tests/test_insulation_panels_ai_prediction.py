...
from app.prediction.insulation_panels_ai import (
    predict_insulation_panels_ai_from_timeline,
)
    ...
    return {
        ...
        "components": {
            "insulation_panels": {
                ...
                "status": "FUNCTIONAL",
                "metrics": {
                    "insulation_efficiency": 1.0 - 0.6 * degradation,
                    "heat_loss_factor": 1.0 + 1.2 * degradation,
                    "panel_integrity": health,
                    "thermal_gradient_c": 18.0
                    * degradation
...
def test_insulation_panels_ai_predictor_trains_model_and_returns_curve():
    result = run_simulation(
        {
            "run_id": "insulation_panels_ai_integration",
            "scenario_id": "ai_test",
            "total_usages": 720,
            "usage_step": 4,
            "initial_conditions": {
            ...
            "selected_component": "insulation_panels",
            "seed": 1234,
        }
    )

    prediction = predict_insulation_panels_ai_from_timeline(
        "insulation_panels_ai_integration",
    ...
    assert prediction["component_id"] == "insulation_panels"
    assert prediction["model_family"] == "insulation_panels_sklearn_gradient_boosting"
    ...
    math_curve = [
        {
            ...
            "health": point["model_output"]["components"]["insulation_panels"][
                "health"
            ],
        }
...
def test_insulation_panels_ai_predicts_more_damage_for_thermal_humid_fouling():
    low_risk_timeline = [
        _timeline_point(
            0,
            0.95,
    ...
    low_risk_prediction = predict_insulation_panels_ai_from_timeline(
        "insulation_low_risk",
        low_risk_timeline,
    )
    high_risk_prediction = predict_insulation_panels_ai_from_timeline(
        "insulation_high_risk",
        high_risk_timeline,
    )
