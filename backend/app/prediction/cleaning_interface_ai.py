"""Scikit-learn AI predictor for the cleaning interface."""
...
COMPONENT_ID = "cleaning_interface"
...
def _metrics_from_state(config: dict, health: float, drivers: dict) -> dict:
    ...
    state = calculate_cleaning_interface_state(
...
def _feature_row(
    ...
    residue_buildup: float,
    wiper_wear_ratio: float,
    wipe_pressure_factor: float,
    ...
    return [
        ...
        float(cleaning_efficiency),
        float(residue_buildup),
        float(wiper_wear_ratio),
        float(wipe_pressure_factor),
...
def _build_training_dataset(config: dict) -> tuple[list[list[float]], list[float]]:
    ...
    usage_values = (0.0, 700.0, 1500.0, 2400.0, 3300.0)
            ...
            for contamination in contamination_values:
                for humidity in humidity_values:
                    for maintenance_level in maintenance_values:
                        ...
                        next_state = calculate_cleaning_interface_state(
                            previous_state={"health": health},
                            drivers=drivers,
                            config=config,
                        )
                        ...
                        for usage_count in usage_values:
                            ...
                            rows.append(
                                _feature_row(
                                    ...
                                    maintenance_level=maintenance_level,
                                    cleaning_efficiency=float(
                                        metrics["cleaning_efficiency"]
                                    ),
                                    residue_buildup=float(
                                        metrics["residue_buildup"]
                                    ),
                                    wiper_wear_ratio=float(
                                        metrics["wiper_wear_ratio"]
                                    ),
                                    wipe_pressure_factor=float(
                                        metrics["wipe_pressure_factor"]
                                    ),
...
def train_cleaning_interface_model() -> tuple[HistGradientBoostingRegressor, dict]:
...
def _build_ai_curve(
    ...
    for point in timeline[1:]:
        ...
        features = _feature_row(
            ...
            cleaning_efficiency=float(metrics["cleaning_efficiency"]),
            residue_buildup=float(metrics["residue_buildup"]),
            wiper_wear_ratio=float(metrics["wiper_wear_ratio"]),
            wipe_pressure_factor=float(metrics["wipe_pressure_factor"]),
...
def predict_cleaning_interface_ai_from_timeline(
    run_id: str,
    timeline: list[dict],
) -> dict:
    points = [
    ...
    if len(points) < 2:
        return {
            ...
            "confidence": 0.2,
            "model_family": "cleaning_interface_sklearn_gradient_boosting",
            "prediction_method": "supervised_synthetic_gradient_boosting",
            "reason": "Not enough cleaning interface history for the AI predictor.",
        }
    ...
    return {
        ...
        "explanation": {
            ...
            "top_factors": [
                ...
                {"name": "residue_buildup", "direction": "risk"},
                {"name": "humidity", "direction": "risk"},
                {"name": "wipe_pressure_factor", "direction": "state"},
