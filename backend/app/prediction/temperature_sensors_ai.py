"""Scikit-learn AI predictor for the temperature sensors."""
...
COMPONENT_ID = "temperature_sensors"
...
def _metrics_from_state(config: dict, health: float, drivers: dict) -> dict:
    ...
    state = calculate_temperature_sensors_state(
...
def _build_training_dataset(config: dict) -> tuple[list[list[float]], list[float]]:
    ...
    usage_values = (0.0, 900.0, 1800.0, 3000.0, 4200.0)
            ...
            for humidity in humidity_values:
                for temperature_stress in temperature_values:
                    for maintenance_level in maintenance_values:
                        ...
                        next_state = calculate_temperature_sensors_state(
                            previous_state={"health": health},
                            drivers=drivers,
                            config=config,
                        )
                        ...
                        for usage_count in usage_values:
                            ...
                            rows.append(
                                _feature_row(
                                    ...
                                    temperature_stress=temperature_stress,
                                    maintenance_level=maintenance_level,
                                    drift_c=float(metrics["drift_c"]),
                                    response_time_ms=float(
                                        metrics["response_time_ms"]
                                    ),
                                    signal_noise_index=float(
                                        metrics["signal_noise_index"]
                                    ),
                                    calibration_confidence=float(
                                        metrics["calibration_confidence"]
                                    ),
...
def train_temperature_sensors_model() -> tuple[HistGradientBoostingRegressor, dict]:
...
def _build_ai_curve(
    ...
    for point in timeline[1:]:
        ...
        features = _feature_row(
            ...
            drift_c=float(metrics["drift_c"]),
            response_time_ms=float(metrics["response_time_ms"]),
            signal_noise_index=float(metrics["signal_noise_index"]),
            calibration_confidence=float(metrics["calibration_confidence"]),
...
def predict_temperature_sensors_ai_from_timeline(
    run_id: str,
    timeline: list[dict],
) -> dict:
    points = [
    ...
    if len(points) < 2:
        return {
            ...
            "confidence": 0.2,
            "model_family": "temperature_sensors_sklearn_gradient_boosting",
            "prediction_method": "supervised_synthetic_gradient_boosting",
            "reason": "Not enough temperature sensor history for the AI predictor.",
        }
    ...
    return {
        ...
        "explanation": {
            ...
            "top_factors": [
                ...
                {"name": "drift_c", "direction": "state"},
                {"name": "calibration_confidence", "direction": "state"},
