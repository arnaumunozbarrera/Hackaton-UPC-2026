...
export default function TimelineChart({ axisData, durationHours, selectedComponentId }) {
  const ticks = buildTicks(durationHours);
    ...
    <section className="panel chart-panel">
      ...
      <div className="chart-wrapper">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={axisData} margin={{ top: 18, right: 14, left: 6, bottom: 12 }}>
            ...
            <XAxis
              ...
              label={{ value: 'Time [h]', position: 'insideBottom', offset: -2, fill: '#6e7681', fontSize: 11 }}
            />
            <YAxis
              domain={[0, 1.02]}
...
function buildTicks(durationHours) {
  const divisions = 6;
  return Array.from({ length: divisions + 1 }, (_, index) => Math.round((durationHours / divisions) * index));
}
